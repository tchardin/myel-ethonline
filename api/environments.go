...
func (pve *providerValidationEnvironment) RunDealDecisioningLogic(ctx context.Context, state rm.ProviderDealState) (bool, string, error) {
	return true, "", nil
}

// StateMachines returns the FSM Group to begin tracking with
func (pve *providerValidationEnvironment) BeginTracking(pds rm.ProviderDealState) error {
	return nil
}
...
func (pre *providerRevalidatorEnvironment) SendEvent(dealID rm.ProviderDealIdentifier, evt rm.ProviderEvent, args ...interface{}) error {
	return pre.p.stateMachines.Send(dealID, evt, args...)
}

func (pre *providerRevalidatorEnvironment) Get(dealID rm.ProviderDealIdentifier) (rm.ProviderDealState, error) {
	var deal rm.ProviderDealState
...
func (psg *providerStoreGetter) Get(otherPeer peer.ID, dealID rm.DealID) (*multistore.Store, error) {
	var deal rm.ProviderDealState
	err := psg.p.stateMachines.GetSync(context.TODO(), rm.ProviderDealIdentifier{Receiver: otherPeer, DealID: dealID}, &deal)
	if err != nil {
		return nil, err
	}
...
func (pde *providerDealEnvironment) TrackTransfer(deal rm.ProviderDealState) error {
	pde.p.revalidator.TrackChannel(deal)
	return nil
}

func (pde *providerDealEnvironment) UntrackTransfer(deal rm.ProviderDealState) error {
