...
func connectToPeers(ctx context.Context, ipfs icore.CoreAPI, peers []string, coPeers chan *peerstore.PeerInfo) error {
	...
	for _, peerInfo := range peerInfos {
		go func(peerInfo *peerstore.PeerInfo) {
			...
			fmt.Printf("Connected to peer %s", peerInfo.ID)
			coPeers <- peerInfo
		}(peerInfo)
	}
	wg.Wait()
...
func SpawnIpfsNode() *C.char {
	...
	fmt.Printf("Node id: %v", node.Identity.String())
...
func GetNodeId() *C.char {
	return C.CString(node.Identity.String())
}
...
func main() {
	...
	fmt.Printf("Result cid: %v", C.GoString(resCid))
